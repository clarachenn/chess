...
void gameClicks() {
  if (itsMyTurn && firstClick) {
    ...
    if (itsMyTurn && !(row2 == row1 && col2 == col1) && p && row2 == 7) {
      grid[row2][col2] = grid[row1][col1];
      grid[row1][col1] = ' ';
      mode = PPROMO;
      myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 4);
      ppro = true;
      firstClick = true;
      itsMyTurn = false;
      sp = false;
...
void keyReleased() {
  if (key == 'z' && !itsMyTurn) {
    if (sp == false) {
      ...
      mode = GAME;
      sp = false;
      myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 8);
      firstClick = true;
      itsMyTurn = true;
      sp = false;
    }
  ...
  if (key == 'q' || key == 'Q' && ppro && p) {
    for (int r = 0; r < 8; r++) {
      for (int c = 0; c < 8; c++) {
        grid[row2][col2] = 'Q';
        mode = WAITING;
        myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 3);
        ppro = false;
        p = false;
        sp = true;
      }
    ...
    for (int r = 0; r < 8; r++) {
      for (int c = 0; c < 8; c++) {
        grid[row2][col2] = 'B';
        mode = WAITING;
        myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 5);
        ppro = false;
        p = false;
        sp = true;
      }
    ...
    for (int r = 0; r < 8; r++) {
      for (int c = 0; c < 8; c++) {
        grid[row2][col2] = 'N';
        mode = WAITING;
        myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 6);
        ppro = false;
        p = false;
        sp = true;
      }
    ...
    for (int r = 0; r < 8; r++) {
      for (int c = 0; c < 8; c++) {
        grid[row2][col2] = 'R';
        mode = WAITING;
        myClient.write(row1 + "," + col1 + "," + row2 + "," + col2 + "," + 7);
        ppro = false;
        p = false;
        sp = true;
      }
